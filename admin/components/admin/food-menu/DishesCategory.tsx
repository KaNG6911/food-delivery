...
export const DishesCategory = () => {
  const { data: categories, isLoading } = useSWR<CategoryWithCount[]>(
    ...
    <div className="flex flex-col gap-4 p-6 bg-background rounded-xl">
      ...
      <div className="flex flex-wrap gap-3">
        <div className="flex gap-2 px-4 py-2 border rounded-full">
          ...
          <p className="text-xs bg-black text-white rounded-full px-2 py-0.5 flex items-center font-semibold leading-4x">
            {allDishesCount}
          </p>
        </div>
        {categories.map((category, index) => (
          <div key={index} className="flex gap-2 px-4 py-2 border rounded-full">
            ...
            <p className="text-xs bg-black text-white rounded-full px-2.5 py-0.5 flex items-center font-semibold leading-4x">
              {category.count}
            </p>
          </div>
        ))}
